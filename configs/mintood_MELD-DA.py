    ...
    def _get_hyper_parameters(self, args):
        ...
        if args.text_backbone.startswith('bert'):
            hyper_parameters = {
                ...
                'base_dim': [768],
                'lr': [4e-6], #3e-5
                'temperature': [1], # bigger is usually better
                'alpha': [0.7], #0.5, 1
                'mlp_hidden_size': [256],
                'mlp_dropout': [0.2],
                're_prob': [0.1],
            ...
            #     'base_dim': [768], # ok
            #     'lr': [3e-6], #
            #     'temperature': [0.5], # bigger is usually better
            #     'alpha': [0.5], #0.5, 1
            #     'mlp_hidden_size': [256],
            #     'mlp_dropout': [0.1],
            #     'num_train_epochs': [100], # ok
            #     'train_batch_size': [32], # ok
            #     'weight_decay': [0.1], # ok
            #     'multiple_ood': [1.0], # try average number
            #     'select_number_min': [2],
            #     'select_number_max': [5],
            ...
            #     'warmup_proportion': [0.1], 
            #     'scale': [16], # ok
            #     'encoder_layers_a': [1], # ok
            #     'encoder_layers_v': [2], # ok
            #     'grad_norm': [-1],
            #     'attn_dropout': [0.2], # ok
            #     'relu_dropout': [0.2], # ok
            #     'embed_dropout': [0],

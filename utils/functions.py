...
def save_results(args, test_results, debug_args = None):
    ...
    _vars = [args.dataset, args.ood_dataset, args.method, args.ablation_type, args.ood_detection_method, args.text_backbone, args.seed, args.log_id]
    _names = ['dataset', 'ood_dataset', 'method', 'ablation_type', 'ood_detection_method', 'text_backbone', 'seed', 'log_id']
...
# def get_mixup_embedding(embs, select_elems):
...
#     if embs.ndim == 2:
#         ood_feats = torch.cat(ood_feats, dim = 0).view(ood_len, -1)
#     else:
#         ood_feats = torch.cat(ood_feats, dim = 0).view(ood_len, embs.shape[1], -1)

#     mixed_embs = torch.cat((embs, ood_feats), dim = 0)

#     return mixed_embs

# def get_mixup_mask(masks, select_elems):

#     idx_a = [e[0] for e in select_elems]
#     idx_b = [e[2] for e in select_elems]

#     ood_masks = []
#     ood_len = masks.shape[0]

#     for i in range(len(idx_a)):
#         mask_a = masks[idx_a[i]]
#         mask_b = masks[idx_b[i]]
#         ood_mask = torch.max(mask_a, mask_b)
#         ood_masks.append(ood_mask)

#     ood_masks = torch.stack(ood_masks, dim=0).view(ood_len, masks.shape[1], masks.shape[2], masks.shape[3])
#     mixed_masks = torch.cat((masks, ood_masks), dim=0)
